...
class OwnerNotification extends Command
{
    ...
    public function getownernotification($job, $fields)
    {
        ...
        if (isset($ownerdetails) && sizeof($ownerdetails) > 0) {
            ...
            $check = OrderDetails::sendownernotification($device_arr, $fields['order_id'], $fields['outlet_id'], $fields['table_no']);
        }
    ...
    public function attendNotification($job, $fields)
    {
        ...
        if (isset($ownerdetails) && sizeof($ownerdetails) > 0) {
            ...
            $check = OrderDetails::sendAttendMeNotification($device_arr, $fields['table_no'], $fields['outlet_id']);
        }
    ...
    public function payBillNotification($job, $fields)
    {
        ...
        if (isset($ownerdetails) && sizeof($ownerdetails) > 0) {
            ...
            $check = OrderDetails::sendPaidBillNotification($device_arr, $fields['table_no'], $fields['outlet_id']);
        }
    ...
    public function toConsumerAcceptKotNotification($job, $fields)
    {
        ...
        $order_details = OrderDetails::where('order_id', $fields['order_id'])->first();

        if (isset($order_details) && sizeof($order_details) > 0) {
            OrderDetails::toConsumerAcceptKotNotification($fields['order_id'], $fields['kot_items'], $order_details->device_id);
        }
    ...
    public function upiTransactionStatusChange($job, $fields)
    {
        ...
        if (isset($fields) && sizeof($fields) > 0) {
            if (isset($fields['billNumber'])) {
                ...
                $order = OrderDetails::where('order_unique_id', $fields['billNumber'])->first();
                ...
                if (isset($order) && sizeof($order) > 0) {
                    ...
                    if (isset($ownerdetails) && sizeof($ownerdetails) > 0) {
                        ...
                        if (isset($device_id) && sizeof($device_id) > 0) {
                            ...
                            OrderDetails::UpiTransactionStatusChangeNotification($device_arr, $order->table_no, $fields['txnId'], $fields['billNumber'], $fields['oldStatus'], $fields['oldStatusString'], $fields['status'], $fields['statusString'], $fields['amount'], $fields['playerVa'], $fields['note'], $fields['initDate']);
                        }
                    }
                }
            }
