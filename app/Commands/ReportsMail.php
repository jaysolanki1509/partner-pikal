    ...
    public function sendmails($job, $fields)
    {
        ...
        if (sizeof($outlets) > 0) {
            ...
            <table class="table table-striped table-bordered table-hover" border="1">
                ...
                $orders = OrderDetails::where('orders.table_start_date', '>=', (new Carbon($from_date))->startOfDay())
                    ->where('orders.table_end_date', '<=', (new Carbon($to_date))->endOfDay())
                    ->where('orders.outlet_id', '=', $outlet->id)
                    ->where('orders.invoice_no', "!=", '')
                    ->where('orders.cancelorder', '!=', 1);

                $orderscount = OrderDetails::where('table_start_date', '>=', (new Carbon($from_date))->startOfDay())
