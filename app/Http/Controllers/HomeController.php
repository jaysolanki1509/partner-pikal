...
class HomeController extends Controller
{
    ...
    public function dashboardOrderData()
    {
        ...
        $date_expense = Expense::whereIn('expense_for', $outlet_ids)->where('expense.expense_date', '>=', $sod)->where('expense.expense_date', '<=', $eod)->where('verify', '1')->sum('expense.amount');
    ...
    public function dashboardreport()
    {
        ...
        // echo "Hello <pre>";print_r($ord); echo "</pre>"; exit;
        return $ord;
    }
    ...
    /* public function populateInvoiceTable()
    {
        ...
        if (isset($orders) && !empty($orders) ) {
            foreach ($orders as $or) {
                ...
                if ( isset($inv_detail) && !empty($inv_detail) ) {
                } else {
                    $inv = new InvoiceDetail();
    ...
    public function populateTaxesPP()
    {
        ...
        foreach ($orders as $ord) {
            $tr = array();
            $total = $ord->totalcost_afterdiscount;
            foreach ($tax as $tx) {
                foreach ($tx as $k => $v) {
                    $tax_arr = array();
                    ...
                    foreach ($outlet as $ot) {
                        ...
                        if (isset($menu) && !empty($menu)) {
                            foreach ($menu as $itm) {
                                ...
                                if (isset($check_itm) && !empty($check_itm)) {
                                } else {
    ...
    public function makeItemisSale()
    {
        ...
        if (isset($menu_bind) && !empty($menu_bind)) {
    ...
    function setNewTax()
    {
        ...
        foreach ($outlet_list as $outlet) {
            if (isset($outlet->taxes) && !empty($outlet->taxes)) {
                $new_taxes = array();
        ...
        if (isset($omb_obj) && !empty($omb_obj) > 0) {
            foreach ($omb_obj  as $omb) {
                ...
                if (isset($menu) && !empty($menu) > 0) {
                    ...
                    $itm_arr = explode(" ", $str);
                    if (isset($itm_arr) && !empty($itm_arr)) {
    ...
    public function deleteOrdersBeforeDate($date)
    {
        ...
        if (isset($sess_outlet_id) && trim($sess_outlet_id) != "") {
            ...
            $order_invoice_count = InvoiceDetail::whereIn('order_id', $order_arr)->count();
            ...
            InvoiceDetail::whereIn('order_id', $order_arr)->delete();
