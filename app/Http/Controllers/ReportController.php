    ...
    public function stockRequestReport(Request $request)
    {
        if ($request->ajax()) {
            ...
            $item_requests = ItemRequest::join('menus', 'menus.id', '=', 'item_request.what_item_id')
                ...
                ->select(
                    'item_request.id as request_id',
                    'item_request.unit_id as unit_id',
                    'item_request.price as price',
                    'item_request.what_item_id',
                    'unit.name as unit_name',
                    'locations.name',
                    'locations.id as loc_id',
                    'owners.user_name',
                    'item_request.location_from',
                    'item_request.what_item',
                    'item_request.owner_to',
                    'item_request.owner_by',
                    'item_request.when',
                    DB::raw('ifnull(sum(item_request.qty),0) as requested_qty'),
                    'item_request.existing_qty',
                    'menus.id',
                    'menus.item',
                    'menus.secondary_units'
                )
    ...
    public function stockResponseReport(Request $request)
    {
        ...
        if ($request->ajax() || (isset($flag) && $flag == 'export')) {
            ...
            $item_requests = ItemRequest::join('unit', 'item_request.satisfied_unit_id', '=', 'unit.id')
                ...
                ->select(
                    'item_request.id as request_id',
                    'item_request.satisfied_unit_id as unit_id',
                    'item_request.price as price',
                    'item_request.what_item_id',
                    'unit.name as unit_name',
                    'locations.name',
                    'locations.id as loc_id',
                    'owners.user_name',
                    'item_request.satisfied',
                    'item_request.location_from',
                    'item_request.what_item',
                    'item_request.satisfied_when',
                    'item_request.owner_to',
                    'item_request.owner_by',
                    'item_request.when',
