...
class Apicontroller extends Controller
{
    ...
    public function owneroutlet()
    {
        ...
        $app_version = Input::json('app_version');
    ...
    public function getnewMenu($resid, $ot_printer = NULL, $flag = NULL)
    {
        ...
        foreach ($menu as $m) {
            ...
            //item option groups
            $item_option_groups = ItemOptionGroupMapper::where('item_id', $m->id)->lists('item_option_group_id');
    ...
    public function matchcouponcode()
    {
        ...
        if (sizeof($coupondata) > 0) {
            $usage_count = OrderCouponMappers::where('coupon_applied', $coupondata->id)->count();
    ...
    public function ordercancellation()
    {
        $resid = Input::json('resid');
        $order_id = Input::json('order_id');
        $neworder_id = Input::json('neworder_id');
    ...
    public function closeCounter(Request $request)
    {
        ...
        $total_cash = Input::json('total_cash');
        $total_online = Input::json('total_online');
        $total_cheque = Input::json('total_cheque');
        ...
        if (isset($outlet) && sizeof($outlet) > 0) {
            ...
            if (isset($check_invoice_no) && sizeof($check_invoice_no) > 0) {
                ...
                foreach ($check_invoice_no as $inv_record) {
                    $inv = (int)substr($inv_record->invoice_no, -1 * $inv_digit);
                    if ($inv > $max_id) {
                        $max_id = $inv;
                    }
                }
    ...
    public function getOnlineOrders()
    {
        ...
        //      $order_id = Input::json('order_id');
        $order_unique_id = Input::json('order_unique_id');
    ...
    public function tableAvailability()
    {
        ...
        $table_no = Input::json('table_no');
        $outlet_id = Input::json('outlet_id');
        $no_of_person = Input::json('table_person');
        $user_id = Input::json('user_id');
        $request_from = Input::json('request_from');
    ...
    public function cancelAllOrder()
    {
        ...
        $user_id = Input::json('user_id');
        $user_name = Input::json('user_name');
        $from = Input::json('order_from');
        $order_type = Input::json('order_type');
        ...
        if ($from == 'consumer') {
            ...
            $table_start_date = Input::json('table_start_date');
            $table_end_date = Input::json('table_end_date');
            $order_unique_id = Input::json('order_unique_id');
    ...
    public function storeRequest()
    {
        ...
        $from_id = Input::json('from_user');
        $to_id = Input::json('to_user');
        $items = Input::json('items');
        $req_date = Input::json('request_date');
        $wrong_request = array();
    ...
    public function processRequest()
    {
        ...
        $data = Input::json('data');
        $user_id = Input::json('user_id');
        $loc_id = Input::json('from_location_id');
        $today = Input::json('response_date');
    ...
    public function updateDeviceId()
    {
        ...
        $user_id = Input::json('user_id');
        $device_id = Input::json('device_id');
        $token_from = Input::json('token_from');
    ...
    public function getAllOutletPush()
    {
        ...
        $fields['device_id'] = Input::get('device_id');
        $fields['flag'] = Input::get('flag');
        $fields['level'] = Input::get('level');
    ...
    public static function uploadLog()
    {
        ...
        $log_array = array();
        $log_array['owner_id'] = Input::get('user_id');
        $log_array['outlet_id'] = Input::get('outlet_id');
        $log_array['app_version'] = Input::get('app_version');
        $log_array['manufacturer'] = Input::get('manufacturer');
        $log_array['model'] = Input::get('model');
        $log_array['os_version'] = Input::get('os_version');
    ...
    public function storeCampaignDetail()
    {
        ...
        $ot_name = Input::get('outlet_name');
        $ow_name = Input::get('owner_name');
    ...
    public function getAllCuisionList()
    {
        ...
        $cuision_types = CuisineType::all()->lists('type', 'id');
        $cuision_array = array();
        $i = 0;
    ...
    public function saveHotKeyConfig()
    {
        ...
        $outlet_id = Input::get("outlet_id");
        $hotkey_config = Input::get("hotkey_config");
