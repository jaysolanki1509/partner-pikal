...
class TablesController extends Controller
{
    ...
    // public function tablesList(GetRequest $request)
    {
        $owner_id = Auth::id();
        ...
        // return Response::json(array('status' => 'success','data' => $tables,'statuscode' => 200,200));
    ...
    {
        $user_id = Auth::id();
        $outlets = OutletMapper::join('outlets as ot', 'ot.id', '=', 'outlets_mapper.outlet_id')->where('outlets_mapper.owner_id', $user_id)->get();
        $outlet_list = array();
    ...
    public function tableStore()
    {
        ...
        if ($validate) {
            ...
            $same_table_number = Tables::where('outlet_id', '=', $outlet_id)->where('table_no', '=', $table_no)->first();
            if (isset($same_table_number) && !empty($same_table_number)) {
                return Response::json(array(
    ...
    public function editForm($id)
    {
        ...
        if (isset($table_details) && !empty($table_details)) {
            return Response::json(array(
                'status' => 'success',
                'data' => $table_details,
                'statuscode' => 200,
    ...
    {
        if (isset($id)) {
            $deleteTables = Tables::find($id);
            $result = $deleteTables->delete();
