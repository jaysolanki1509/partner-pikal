<?php use App\OrderItem; if(count($orders)>0) { ?>
    ...
    <style>
        ...
        .delivered{background: #ce93d8;margin:15px;}
        .ajax-loader {
            position:absolute;
            top:0; bottom:0;
            left:0; right:0;
    ...
    else if($allorder->status=="delivered"){
        $class="delivered";
    }
    $oid=$allorder->suborder_id;
    DB::table('orders')->where('suborder_id',$oid)->where('outlet_id',$allorder->outlet_id)->update(array("read"=>1));
    ?>
    ...
    <div class="bs-example col-md-4" style="min-height:240px;max-height:240px;overflow: scroll;word-wrap:break-word;border-style:solid;border-width: 1px;padding: 0 0 0 0;margin-left:5px;margin-right:5px;margin-bottom:10px;border-color: #ddd;">
                ...
                <div id="collapseOne{{$oid}}" class="panel-collapse collapse ">
                    <div class="panel-body">
                        ...
                        <table id="orderdet" class="col-xs-2 col-md-4 getorderdetails ">
                            ...
                            <tr>
                                <td style="padding:10px;">{{ trans('Order.Name') }}  </td>
                                <td>{{$allorder->name}}</td>
                            </tr>
                            <tr>
                                <td style="padding:10px;">{{ trans('Order.Address') }}  </td>
                                <td><address>{{$allorder->address}}</address></td>
                            </tr>
                            <tr>
                                <td style="padding:10px;">{{ trans('Order.Mobile') }}  </td>
        ...
        <div class="panel-group" id="accordion{{$oid}}new" style="margin:0 0 0 0 !important;">
            <div class="panel panel-default" style="border-bottom-style: none;">
                <div class="panel-heading">
                    <h4 class="panel-title">
                        <a data-toggle="collapse" data-parent="#accordion{{$oid}}new" href="#collapseOne{{$oid}}new" class="coldown">{{ trans('Order.ITEM') }}</a>
                    </h4>
                </div>
                ...
                <div id="collapseOne{{$oid}}new" class="panel-collapse collapse in" >
                    <div class="panel-body" style="padding: 0 0 0 0;" >
                        ...
                        //print_r($item);
                            ...
                            <tr>
                                <th style="padding:10px;">{{ trans('Order.ITEM') }}</th>
                                <th style="padding:10px;text-align:center;">{{ trans('Order.Quantity') }}</th>
                       ...
                       if(isset($getmenu['price']) && $getmenu['price']!=""){
                           ...
                           $totalprice='';
                            ...
                            @endforeach
                            <tr>
                                <td style="padding:10px;"><b>{{ trans('Order.Total Price:') }}</b></td><td>&nbsp;</td><td style="padding:10px;text-align: center;">{{$totalprice}}</td></tr>
                            <input type="hidden" id="orderstatusget" value="{{$allorder->status}}">
                            <tr>
                                <td style="padding:10px;">
                                    <?php if($allorder->status!="delivered"){ ?><button class="btn btn-primary" onclick='changestatus("{{$allorder->status}}","{{$oid}}");'>{{ trans('OrderAdd.Next') }}</button><?php } ?></td>
                            </tr>
                        </table>
                    </div>
                </div>
